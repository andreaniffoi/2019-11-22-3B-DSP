...
```F.c.>2Fmax
'''la teoria del campionamento serve qualora si voglia produrre o registrare un suono. L'ampiezza del suono desiderato deve calcolata per un numero di volte maggiore al doppio della sua frequenza
```
...
```F.c.= _1520.1Hz_
'''vado a calcolare la frequenza del settimo armonico moltiplicando la fondamentale *(7), vado a definire un numero maggiore rispetto al dopiio della frequenza del settimo armonico
```
...
;
```import("stdfaust.lib");
process = *(0.5) , *(0.25) , *(0.125) , *(0.1) <: _ , *(1) , _ , *(1) , _ , *(1) , _ , *(1) ;
